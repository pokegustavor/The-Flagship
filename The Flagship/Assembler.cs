...
using System.IO;
using static The_Flagship.Command;
...
namespace The_Flagship
{
    internal class Assembler
    {
        ...
        public static async void FabricateFlagship()
        {
            ...
            PLShipInfo ship = prevShip;
            if (prevShip.ShipTypeID != EShipType.OLDWARS_HUMAN)
            {
                if (PhotonNetwork.isMasterClient)
                {
                    //Spawn interceptor for asset ripping
                    PLPersistantShipInfo plpersistantShipInfo = new PLPersistantShipInfo(EShipType.OLDWARS_HUMAN, 0, PLServer.GetCurrentSector(), 0, false, false, true, -1, -1)
                    {
                        ShipName = prevShip.ShipName
                    };
                    PLServer.Instance.AllPSIs.Add(plpersistantShipInfo);
                    ship = PLEncounterManager.Instance.GetCPEI().SpawnEnemyShip(plpersistantShipInfo.Type, plpersistantShipInfo, PLEncounterManager.Instance.PlayerShip.Exterior.transform.position + new Vector3(0, 200, 0), default) as PLShipInfo;
                    ...
                    PLServer.Instance.photonView.RPC("ClaimShip", PhotonTargets.All, new object[] { ship.ShipID });
                    PLServer.Instance.LongRangeCommsDisabled = renable;
                }
                else 
                {
                    //wait for host to spawn interceptor
                    do
                    {
                        await Task.Yield();
                    } while (PLEncounterManager.Instance.PlayerShip == null || PLEncounterManager.Instance.PlayerShip.ShipTypeID != EShipType.OLDWARS_HUMAN);
                    ship = PLEncounterManager.Instance.PlayerShip;
                    PulsarModLoader.Utilities.Messaging.Notification("Y value: " + ship.InteriorStatic.transform.position.y);
                }
            }
            ship.IsGodModeActive = true;
            prevShip.IsGodModeActive = true;
            ship.FactionID = 1;
            ship.ShipName = prevShip.ShipName;
            float verticalOffset = ship.InteriorStatic.transform.position.y + 400;


            //Load sectors that are needed for asset ripping
            List<AsyncOperation> ops = new List<AsyncOperation>();
            List<int> sectorsNeeded = new List<int>();
            if (currentSector.VisualIndication != ESectorVisualIndication.AOG_HUB)
            {
                sectorsNeeded.Add(67);
            }
            if (currentSector.VisualIndication != ESectorVisualIndication.WD_START)
            {
                sectorsNeeded.Add(10);
            }
            if (currentSector.VisualIndication != ESectorVisualIndication.WD_INFECTEDFLAGSHIP)
            {
                sectorsNeeded.Add(105);
            }
            if (currentSector.VisualIndication != ESectorVisualIndication.MANOR_STATION)
            {
                sectorsNeeded.Add(98);
            }
            foreach (int needs in sectorsNeeded)
            {
                ops.Add(SceneManager.LoadSceneAsync(needs, LoadSceneMode.Additive));
            }
            ...
            while (!fullyLoaded)
            {
                ...
                if (!fullyLoaded) await Task.Yield();
            }
            ops.Clear();
            List<MeshRenderer> camerasRenders = new List<MeshRenderer>();
            //Assemble Exterior
            PulsarModLoader.Utilities.Logger.Info("Phase 1");
            if (ship != null)
            {
                AssembleExterior(ship);

                GameObject oldInterior = RemoveOldInterior(ship.InteriorStatic);
                ...
                GameObject startup_screen = null;
                ...
                //Get all assets needed
                PulsarModLoader.Utilities.Logger.Info("Phase 2");
                foreach (GameObject gameObject in Object.FindObjectsOfType<GameObject>(true))
                {
                    ...
                    else if (gameObject.name == "Turrett_Control_01" && smallturret1 == null && gameObject != smallturret2 && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        smallturret1 = gameObject;
                    }
                    else if (gameObject.name == "Turrett_Control_01" && gameObject != smallturret1 && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        smallturret2 = gameObject;
                    }
                    else if (gameObject.name == "Turrett_Control_02" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        mainturret = gameObject;
                    }
                    else if (gameObject.name == "SystemInstance_Weapons" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        weaponssys = gameObject;
                    }
                    else if (gameObject.name == "Switch_01" && gameObject.transform.childCount > 0 && gameObject.transform.GetChild(0).name == "pCube24" && nukeswitch1 == null && gameObject != nukeswitch2 && gameObject.transform.childCount == 3 && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        nukeswitch1 = gameObject;
                    }
                    else if (gameObject.name == "Switch_01" && gameObject.transform.childCount > 0 && gameObject.transform.GetChild(0).name == "pCube24" && gameObject != nukeswitch1 && gameObject.transform.childCount == 3 && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        nukeswitch2 = gameObject;
                    }
                    else if (gameObject.name == "Nuke_Desk_02" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        nukecore = gameObject;
                    }
                    else if (gameObject.name == "LifeSupportSysInstance" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        lifesys = gameObject;
                    }
                    else if (gameObject.name == "ComputerSysInstance" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        sciencesys = gameObject;
                    }
                    else if (gameObject.name == "FuelBoard" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        fuelboard = gameObject;
                    }
                    else if (gameObject.name == "Decal_ManualProgramCharge" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                    ...
                    else if (gameObject.name == "EngineeringSysInstance" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        enginesys = gameObject;
                    }
                    else if (gameObject.name == "Switchboard_01" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        switchboard = gameObject;
                    }
                    else if (gameObject.name == "Startup_Screen" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        startup_screen = gameObject;
                    }
                    else if (gameObject.name == "Switch_01" && gameObject.transform.childCount == 4 && powerswitches[0] == null && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        powerswitches[0] = gameObject;
                    }
                    else if (gameObject.name == "Switch_01" && gameObject.transform.childCount == 4 && powerswitches[1] == null && gameObject != powerswitches[0] && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        powerswitches[1] = gameObject;
                    }
                    else if (gameObject.name == "Switch_01" && gameObject.transform.childCount == 4 && powerswitches[2] == null && gameObject != powerswitches[1] && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                    ...
                    else if (gameObject.name == "Lever_01" && gameObject.GetComponent<PLReactorCoreEjectStation>() != null && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        ejectswitch = gameObject;
                    }
                    else if (gameObject.name == "Quad" && gameObject.layer == ship.InteriorStatic.layer && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        ejectlabel = gameObject;
                    }
                    else if (gameObject.name == "Lever_01" && gameObject.GetComponent<PLReactorSafetyPanel>() != null && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        safetyswitch = gameObject;
                    }
                    else if (gameObject.name == "SafetySwitchboard_01" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                        safetybox = gameObject;
                    }
                    else if (gameObject.name == "Decal_CoreSafety" && gameObject.GetComponentInParent<PLShipConnection>() != null && gameObject.GetComponentInParent<PLShipConnection>().MyShip is PLOldWarsShip_Human)
                    {
                    ...
                    else if (gameObject.name == "Quad" && gameObject.scene.buildIndex == 10)
                    {
                        blackbox = gameObject;
                    }
                    ...
                    if (interior != null && bridge != null && rightwing != null && rightwingDeco != null && vault != null && vaultDeco != null && engineering != null && reactorroom != null && copydoor != null
                        ...
                        && safetylabel != null && teldoor != null && barber != null && neural != null && landdrone != null && blackbox != null && foxplush != null && walkway != null && fighterCargo != null && fighterCargoBase != null && startup_screen != null) break;
                }
                //Assemble interior
                PulsarModLoader.Utilities.Logger.Info("Phase 3");
                if (interior != null && bridge != null && rightwing != null && rightwingDeco != null && vault != null && vaultDeco != null && engineering != null)
                {
                    ...
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.1");
                    ...
                    */
                    ...
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.2");
                    ...
                    foreach (Transform transform in newinterior.transform.GetChild(5))
                    {
                        if (transform.gameObject.name.Contains("FS_Bar_Brig_Deco_02"))
                        {
                            ...
                            {
                                Command.prisionCells[0] = transform.gameObject;
                                continue;
                            }
                            for (int i = 1; i < 10; i++)
                            {
                                ...
                                {
                                    Command.prisionCells[i] = transform.gameObject;
                                    break;
                                }
                            }
                        }
                    ...
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.3");
                    //remove infected sounds
                    ...
                    //Move captain chair position
                    ship.CaptainsChairPivot.position = new Vector3(-0.7818f, -261.7534f, -324.4219f);
                    ship.CaptainsChairPivot.SetParent(newbridge.transform);
                    ...
                    //Enable teleportation doors
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.4");
                    AssembleInteriorDoors(ship, newinterior, newbridge);
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.5");
                    ...
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.6");
                    newinterior.AddComponent<PLShipConnection>();
                    newinterior.GetComponent<PLShipConnection>().MyShip = ship;
                    AssembleNavMesh(ship, newinterior, newbridge);
                    //Assemble fighter cargo pads
                    ...
                    //Move Interactables
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.7");
                    List<Transform> allturrets = new List<Transform>();
                    //Small turret control stations
                    if (smallturret1 != null)
                    {
                        ...
                        smallturret1.transform.SetParent(newinterior.transform, true);
                    ...
                    if (smallturret2 != null)
                    {
                        ...
                        smallturret2.transform.SetParent(newinterior.transform, true);
                    ...
                    //Main turret control station
                    if (mainturret != null)
                    {
                        mainturret.transform.SetParent(newinterior.transform, true);
                    ...
                    if (weaponssys != null)
                    {
                        weaponssys.transform.SetParent(newinterior.transform, true);
                    ...
                    if (nukecore != null && nukeswitch1 != null && nukeswitch2 != null)
                    {
                        ...
                        nukecore.transform.SetParent(newinterior.transform, true);
                    }
                    //Life support system
                    if (lifesys != null)
                    {
                        lifesys.transform.SetParent(newinterior.transform, true);
                    ...
                    if (sciencesys != null)
                    {
                        sciencesys.transform.SetParent(newinterior.transform, true);
                    ...
                    if (enginesys != null)
                    {
                        enginesys.transform.SetParent(newinterior.transform, true);
                    ...
                    if (fuelboard != null)
                    {
                        ship.WarpFuelBoostLever.transform.SetParent(newinterior.transform, true);
                        ...
                        fuelboard.transform.SetParent(newinterior.transform, true);
                        ...
                        if (fueldecal != null)
                        {
                            fueldecal.transform.SetParent(newinterior.transform, true);
                    ...
                    if (switchboard != null)
                    {
                        if (startup_screen != null) 
                        {
                            startup_screen.transform.SetParent(switchboard.transform);
                        }
                        switchboard.transform.SetParent(newinterior.transform, true);
                        ...
                        {
                            powerswitches[0].transform.SetParent(newinterior.transform, true);
                        ...
                        {
                            powerswitches[1].transform.SetParent(newinterior.transform, true);
                        ...
                        {
                            powerswitches[2].transform.SetParent(newinterior.transform, true);
                    ...
                    if (safetybox != null)
                    {
                        ...
                        safetybox.transform.SetParent(newinterior.transform, true);
                    ...
                    if (chair != null)
                    {
                        SetupLiarsDice(ship, chair,newinterior);
                    }
                    //Core eject lever
                    if (ejectswitch != null)
                    {
                        ...
                        ejectswitch.transform.SetParent(newinterior.transform, true);
                    ...
                    //Move ammo refill station
                    ship.MyAmmoRefills[0].transform.SetParent(newinterior.transform, true);
                    ...
                    //Move crew spawn points
                    (ship.Spawners[0] as GameObject).transform.position = new Vector3(-2, -260, -324);
                    (ship.Spawners[0] as GameObject).transform.SetParent(newbridge.transform);
                    (ship.Spawners[1] as GameObject).transform.position = new Vector3(0, -261, -318.7f);
                    (ship.Spawners[1] as GameObject).transform.SetParent(newbridge.transform);
                    (ship.Spawners[2] as GameObject).transform.position = new Vector3(-1, -260, -322);
                    (ship.Spawners[2] as GameObject).transform.SetParent(newbridge.transform);
                    (ship.Spawners[3] as GameObject).transform.position = new Vector3(-2, -260, -325);
                    (ship.Spawners[3] as GameObject).transform.SetParent(newbridge.transform);
                    (ship.Spawners[4] as GameObject).transform.position = new Vector3(0, -260, -326);
                    (ship.Spawners[4] as GameObject).transform.SetParent(newbridge.transform);
                    (ship.Spawners[4] as GameObject).transform.rotation = new Quaternion(0, 0, 0, 1);
                    //Move log station
                    ship.ShipLogStation.transform.SetParent(newinterior.transform, true);
                    ...
                    ship.GetLockers()[0].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.GetLockers()[1].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.GetLockers()[2].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.GetLockers()[3].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.GetLockers()[4].transform.SetParent(newinterior.transform, true);
                    //Move subsystems screens
                    ...
                    ship.BridgeCameraTransform.SetParent(newbridge.transform);
                    //Move Screens
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.8");
                    AssembleScreens(ship, prevShip, verticalOffset);
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.9");
                    ...
                    //Add bridge comms screen
                    ship.DialogueChoiceBG.transform.position = new Vector3(0.7823f, -259 + verticalOffset, -321.5387f);
                    ship.DialogueChoiceBG.transform.rotation = new Quaternion(-0.1016f, 0.1994f, 0.0202f, 0.9744f);
                    ship.DialogueTextBG.transform.position = new Vector3(-2.7849f, -259 + verticalOffset, -321.5387f);
                    ship.DialogueTextBG.transform.rotation = new Quaternion(0.0861f, 0.2357f, 0.0244f, -0.9672f);
                    //Add new teleporter locations
                    ship.MyTLI.AllTTIs[0].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.ExosuitVisualAssets[0].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.ExosuitVisualAssets[1].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.ExosuitVisualAssets[2].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.ExosuitVisualAssets[3].transform.SetParent(newinterior.transform, true);
                    ...
                    ship.ExosuitVisualAssets[4].transform.SetParent(newinterior.transform, true);
                    //Add atriums
                    ...
                    //Add research pickups to garden
                    ...
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.10");
                    ...
                    droneUpgradeRoot = Object.Instantiate(ship.EngUpgradeUIWorldRoot.gameObject, new Vector3(-12.7774f, -259.9289f + verticalOffset, -337.9999f), Quaternion.Euler(new Vector3(0, 180, 0)));
                    ...
                    if (foxplush != null)
                    {
                        AssembleFoxes(newinterior, foxplush);
                    }
                    //Setup Camera system
                    PulsarModLoader.Utilities.Logger.Info("Phase 3.11");
                    ship.ReactorInstance.transform.SetParent(newinterior.transform, true);
                    ...
                    if (PLNetworkManager.Instance.MyLocalPawn != null)
                    {
                        ...
                        PLNetworkManager.Instance.LocalPlayer.CurrentInterior = newbridge.GetComponent<PLInterior>();
                        PLNetworkManager.Instance.MyLocalPawn.transform.position = (ship.Spawners[PLNetworkManager.Instance.LocalPlayer.GetClassID()] as GameObject).transform.position;
                    }
                    Object.DestroyImmediate(oldInterior);
                }
            }

            PulsarModLoader.Utilities.Logger.Info("Phase 4");
            //unload sectors that were loaded
            foreach (int needs in sectorsNeeded)
            {
                ops.Add(SceneManager.UnloadSceneAsync(SceneManager.GetSceneByBuildIndex(needs)));
            }
            fullyLoaded = false;
            while (!fullyLoaded)
            {
            ...
            PulsarModLoader.Utilities.Logger.Info("Phase 5");
        ...
        static void MoveObjAndChildrenToLayer(GameObject obj, int layer) 
        {
            obj.layer = layer;
            foreach (Transform trans in obj.transform) 
            {
                MoveObjAndChildrenToLayer(trans.gameObject,layer);
            }
        }
        static void AssembleExterior(PLShipInfo ship)
        {
            ...
            {
                if (gameObject.name == "Test_Exterior" && gameObject.scene.buildIndex == 67)
                {
                    exterior = gameObject;
                    break;
                }
        ...
        static void AssembleCameras(GameObject newinterior, GameObject newbridge, PLPlayer payer ,ref List<MeshRenderer> camerasRenders) 
        {
            ...
            cameraObj.name = "cameraObj";
            cameraObj.transform.SetParent(newinterior.transform,true);
            ...
            head.transform.GetChild(0).gameObject.layer = newbridge.layer;
            Object.DontDestroyOnLoad(head);
            head.transform.SetParent(newinterior.transform,true);
            head.transform.localScale = new Vector3(0.3f, 0.3f, 0.3f);
            //Bridge kitchen
            cameraObj = GameObject.CreatePrimitive(PrimitiveType.Cube);
            cameraObj.name = "cameraObj";
        ...
        static void AssembleInteriorDoors(PLShipInfo ship, GameObject newinterior, GameObject newbridge) 
        {
            ...
            if (CapitanToBridge != null && BridgeToCaptain != null)
            {
                ...
                BridgeToScienceOjb.transform.SetParent(newbridge.transform);
                ...
                WeaponsToBridgeOjb.transform.SetParent(newbridge.transform);
        ...
        static void AssembleScreens(PLShipInfo ship, PLShipInfo prevShip, float verticalOffset) 
        {
            ...
            {
                sensorscreen.transform.position = new Vector3(-1.1564f, -260.2763f + verticalOffset, -321.4911f);
            ...
            {
                computerscreen.transform.position = new Vector3(-0.1764f, -260.2763f + verticalOffset, -321.4911f);
            ...
            if (virusscreen == null)
            {
                foreach (PLUIScreen screen in prevShip.MyScreenBase.AllScreens)
                {
                    if (screen is PLScientistVirusScreen)
                    {
                        virusscreen = screen as PLScientistVirusScreen;
                        virusscreen.transform.SetParent(ship.InteriorDynamic.transform);
                        virusscreen.MyScreenHubBase = ship.MyScreenBase;
                        ship.MyScreenBase.AllScreens.Add(virusscreen);
                        Object.DestroyImmediate(virusscreen.gameObject.GetComponent<Light>());
                        virusscreen.MyRootPanel = null;
                        virusscreen.Start();
                        virusscreen.SetupUI();
                        virusscreen.transform.localScale = new Vector3(1.5f, 1.5f, 1.5f);
                        break;
                    }
                }
            }
            if (virusscreen != null)
            {
                virusscreen.transform.position = new Vector3(0.8726f, -260.2763f + verticalOffset, -321.88f);
            ...
            if (commsScreen != null)
            {
                ...
                commsScreen.transform.position = new Vector3(-2.1074f, -260.276f + verticalOffset, -321.7779f);
                commsScreen.transform.rotation = new Quaternion(0, 0.9728f, 0, 0.2318f);
            }
            if (statusscreen != null)
            {

                statusscreen.transform.position = new Vector3(-3.0238f, -260.276f + verticalOffset, -322.3781f);
                statusscreen.transform.rotation = new Quaternion(0, 0.9065f, 0, 0.4222f);
            }
            if (reactorscreen != null)
            {

                reactorscreen.transform.position = new Vector3(-0.3147f, -260.276f + verticalOffset, -327.1871f);
            ...
            if (warpscreen != null)
            {
                warpscreen.transform.position = new Vector3(-1.3783f, -260.276f + verticalOffset, -327.1472f);
            ...
            if (coolantscreen != null) //Also Adding modded screens here, because why not
            {
                ...
                coolantscreen.transform.position = new Vector3(0.6817f, -260.276f + verticalOffset, -326.9217f);
            ...
            if (auxscreen != null)
            {
                ...
                auxscreen.transform.position = new Vector3(-2.2474f, -260.276f + verticalOffset, -326.8127f);
            ...
            if (pilotscren != null)
            {
                ...
                pilotscren.transform.position = new Vector3(-1.5436f, -260.6036f + verticalOffset, -316.256f);
            }
            if (clonedScreen != null)
            {

                clonedScreen.transform.position = new Vector3(0.0273f, -260.6036f + verticalOffset, -316.256f);
        ...
        static void AssembleNavMesh(PLShipInfo ship, GameObject newinterior, GameObject newbridge) 
        {
        ...
        static void AssembleFoxes(GameObject newinterior, GameObject foxplush) 
        {
        ...
        static void SetupLiarsDice(PLShipInfo ship, GameObject chair, GameObject newinterior) 
        {
            ...
            cloneChair.transform.SetParent(newinterior.transform, true);
            ...
            ship.GetLiarsDice().transform.SetParent(newinterior.transform, true);
            ...
            ship.LiarsDice_SitPositions[0].transform.SetParent(newinterior.transform, true);
            ...
            ship.LiarsDice_SitPositions[1].transform.SetParent(newinterior.transform, true);
            ...
            ship.LiarsDice_SitPositions[2].transform.SetParent(newinterior.transform, true);
            ...
            ship.LiarsDice_SitPositions[3].transform.SetParent(newinterior.transform, true);
            ...
            ship.LiarsDice_SitPositions[4].transform.SetParent(newinterior.transform, true);
        }
        static GameObject RemoveOldInterior(GameObject Interior) 
        {
            GameObject obj = Interior.transform.Find("OW_HumanShip_Interior").gameObject;
            obj.SetActive(false);
            Interior.transform.Find("BRIDGE_WEAP_CARGO_ROOMS").SetParent(obj.transform);
            Interior.transform.Find("CREW_BATH_ROOMS").SetParent(obj.transform);
            Interior.transform.Find("TELEPORTER_ROOM").SetParent(obj.transform);
            Interior.transform.Find("LIFE_SCI_CAPT_ROOMS").SetParent(obj.transform);
            Interior.transform.Find("ENGINE_ROOM").SetParent(obj.transform);
            return obj;
        }
        static async void SpawnPatrolBots(PLShipInfo ship, float verticalOffset) 
        {
            List<PLCombatTarget> drones = new List<PLCombatTarget>();
            Vector3[] positions = new Vector3[]
            {
                new Vector3(0, -261 + verticalOffset, -411),
        ...
        static async void SpawnTurrets(PLShipInfo ship) 
        {
            List<PLCombatTarget> drones = new List<PLCombatTarget>();
            Vector3[] positions = new Vector3[]
            {
            ...
            foreach (Vector3 vector in positions)
            {
                PLCombatTarget component = PhotonNetwork.Instantiate("NetworkPrefabs/GroundTurret", vector, Quaternion.identity, 0, null).GetComponent<PLCombatTarget>();
                if (component != null)
                {
                    MoveObjAndChildrenToLayer(component.gameObject, 11);
                    component.CurrentShip = ship;
                    component.MyCurrentTLI = ship.MyTLI;
                    drones.Add(component);
                }
            }
            await Task.Delay(100 * drones.Count);
